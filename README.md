...
**作者**：黄徽徽 · 联系方式：H15217830799 · 项目：链接提取工具 v1.6.0
...
**核心能力**：
- 粘贴抖音/小红书分享链接（支持整段混合文本）→ 自动提取完整的链接和文案
- 生成**转换链接**（去渠道参数）+ 提取**文案/作者/时间/点赞/封面**
- **逐条独立复制**，适配一条条填表
- 历史记录 + 统计看板，**设备 ID 隔离**（每人只见自己的数据）
- 纯 Python 实现，无浏览器依赖，低内存服务器可跑

**稳定性设计**：小红书使用页面主状态、JSON-LD 与 meta 多层解析；临时空页面会以新会话有限重试，登录页和过期分享凭证会快速给出可操作提示。完整结果按作品 ID 短期复用，减少重复请求平台。

**适用人群**：自媒体创作者、内容运营、需要批量整理平台作品链接的个人或小团队。

## 使用免责声明

本工具仅用于链接整理和信息辅助提取，不保证提取结果 100% 准确、完整、及时或持续有效。由于平台页面结构、分享链接、访问权限和网络环境可能发生变化，提取结果可能缺失、错误或失效。使用者在复制、发布、提交或据此开展其他操作前，应自行逐条核对链接、文案、作者、时间、点赞数、封面等信息。因使用本工具、依赖提取结果或未核对结果所产生的任何后果，由使用者自行判断并承担。请遵守相关平台规则、法律法规及内容权利人的要求，不要高频抓取或滥用本工具。
...
- 🧭 可诊断失败：区分链接无效、作品不可见、平台临时异常和分享凭证失效；后台可查看失败原因聚合
...
|----|------|
| 后端 | Python Flask |
| 提取 | 抖音：移动端分享页 `_ROUTER_DATA` JSON 解析；小红书：`__INITIAL_STATE__` 主解析 + JSON-LD/meta 兜底 + 新会话有限重试 |
...
成功且正文完整的结果会缓存 24 小时，跨服务进程和重启有效；抖音、小红书可按稳定作品 ID 复用完整内容。小红书共享缓存不会保存用户分享参数。DNS 公网校验缓存 60 秒，仍会定期重新校验。管理员可调用 `/api/admin/performance` 查看平均耗时和缓存命中率。
...
1. **反爬风险**：在线部署后抖音/小红书看到的是数据中心 IP，提取成功率可能低于本机；若频繁失败，建议限制使用频率（已内置每 IP 20 次/分钟和小红书请求节流）
2. **小红书分享链接会过期**：请用小红书 App「复制链接」功能获取包含 `xsec_token` 的最新链接；若服务提示登录页或凭证失效，重新复制后再试
