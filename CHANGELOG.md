...
### 修复
- **转化成功率可超过 100% 的严重统计 bug**：`api_admin_overview` 风控判定循环中局部变量 `total = h["total"]` 与总览 `total`（全平台非 blocked 记录数）**变量名冲突**，循环执行后把顶层 `total` 覆盖成最后一个平台的记录数，导致 `success_rate = ok_count / total` 计算异常（实测 244%，因 total 被覆盖成小红书单平台数 450 而 ok_count 为全平台 1100）。修复：循环内局部变量改名 `plat_total`，与总览 `total` 解耦。
- **平台健康度/平台分布/趋势分线统计拆组**：历史与新数据 `platform` 字段值不统一（新版落库英文 `douyin`/`xiaohongshu`，旧数据为中文「抖音/小红书」），导致同一平台被拆成两组（如「抖音」649 与「douyin」33、两个「小红书」）。修复：`platform_dist` / `platform_health` / `trend` 的平台判定统一归一化中英文值，同平台合并统计。
...
- **前后端重试冷却对齐（30s）**：前端重试节流由 15s 上调至 **30s**，与后端同 URL 冷却（`_URL_COOLDOWN_SECONDS=30`）保持一致；后端冷却拦截响应新增 `cooldown_remaining` 剩余秒数，前端收到后**优先采用后端剩余时间**对齐按钮倒计时，彻底消除「前端提示可重试、后端仍拦截」的口径割裂。前端批量重试 `retryFailedLinks` 同步跳过 `cached_fail` 条目——确定性失败（链接无效/凭证失效/残缺）10 分钟内重试无意义，引导用户重新复制链接而非盲目 force 重打平台。

### 变更
- **看板风控判定收敛误报**：risk 判定由「总失败率（含用户输入错误）」改为「服务失败占比（仅 upstream_error/internal_error）」——此前「未知平台」全是用户乱粘的无效链接（invalid_input）时会误报「平台风控或上游异常」红色告警；修正后仅上游/内部错误占比 ≥30%（risk）/ ≥15%（warn）才告警，用户输入错误不再触发平台风控告警。疑似风控错误聚类（suspect_count）同步限定为 `upstream_error`/`internal_error`，error 文本关键词（验证/暂时限制访问等）不再对用户输入错误生效，防止未来提示文案含关键词导致误报。
